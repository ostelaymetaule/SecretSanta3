...
namespace SecretSanta.Bot.Helpers
{
    public class BotCaller
    {
        ...
        {
            var keyboard = new InlineKeyboardMarkup(options.Select(x => new[] { new InlineKeyboardButton() }).ToArray());
        ...
        public async Task<bool> SendNotificationAsync()
        {
            ...
            foreach (var infoUser in userInfos)
            {
                if (!infoUser.NotificationSent)
                {
                    ...
                    {
                        await _client.SendTextMessageAsync(infoUser.ChatId, "Привет! Мы заканчиваем прием заявок. Ты не указал адрес и ФИО куда подарок слать. До 31.11.17 можешь передумать и заполнить анкету. Иначе при распределении дедов морозов твоя заявка учитываться не будет.");
                    }
                    else
                    {
                        await _client.SendTextMessageAsync(infoUser.ChatId, "Привет! Мы заканчиваем прием заявок. Вот что ты указал в анкете. Пожалуйста проверь, всё ли верно! 01.12.17 великий рандом подберет тебе внучка.");
